...
tusb_desc_device_t desc_device = {
    ...
    .idVendor = USB_VID,
    .idProduct = USB_PID,
    .bcdDevice = 0x7705, // Mudamos para 05 para forçar o Windows a limpar o cache novamente!
...
// ---------------------------------------------------------------------
// Descriptores: Limitados a 2 interfaces para respeitar o limite do firmware
// ---------------------------------------------------------------------

// Interface 0: Apenas Mouse G600
...
// Interface 1: Teclado (ID 2) + Vendor (ID 128+) + Web Config (ID 100/101)
const uint8_t desc_g600_itf1[] = {
    ...
    // -- REMAPPER WEB CONFIG (Usará os IDs 100 e 101 invisíveis para o mouse original) --
...
const uint8_t configuration_descriptor_g600[] = {
    // Config number, interface count (2 interfaces max!), string index, total length, attribute, power in mA
    TUD_CONFIG_DESCRIPTOR(1, 2, 4, TUD_CONFIG_DESC_LEN + (TUD_HID_DESC_LEN * 2), TUSB_DESC_CONFIG_ATT_REMOTE_WAKEUP, 250),
    
    // Interface 0: Mouse
    TUD_HID_DESCRIPTOR(0, 0, HID_ITF_PROTOCOL_MOUSE, sizeof(desc_g600_mouse), 0x81, CFG_TUD_HID_EP_BUFSIZE, 1),
    
    // Interface 1: Teclado + Vendor + Web Config (Tudo agrupado)
    TUD_HID_DESCRIPTOR(1, 0, HID_ITF_PROTOCOL_KEYBOARD, sizeof(desc_g600_itf1), 0x82, CFG_TUD_HID_EP_BUFSIZE, 1),
};
...
// O Firmware usa o ID do report para saber se é entrada ou se é configuração web
